...
class DroneManager:
    ...
    def _format_cap(self, cap: int) -> str:
        """Formate la capacité : remplace la valeur illimitée 999999 par le nombre total de drones."""
        if cap >= 999999:
            return str(len(self.drones))
        return str(cap)

    # def print_schedule(self) -> None:
    #     """Affiche le planning détaillé avec capacités dans le terminal."""
    #     start_zone = self.network.start
    #     drone_positions: dict[str, str] = {d.id_drone: start_zone for d in self.drones}

    #     for round_idx in range(len(self.schedule)):
    #         line = self.format_round(round_idx, drone_positions)
    #         if line:
    #             print(line)
    def print_schedule(self) -> None:
        """Affichage standard et épuré dans le terminal."""
        for i, round_moves in enumerate(self.schedule, 1):
            # move[0] correspond à id_drone, move[2] à zone_b
            line = " ".join(f"{move[0]}-{move[2]}" for move in round_moves)
            print(f"Turn {i}   -- {line}")

    def format_round(self, round_idx: int, drone_positions: dict[str, str]) -> str:
        """Méthode gardée en réserve pour le live coding si l'évaluation exige le format détaillé."""
        ...
        for id_drone, z_from, z_to, link_name, is_link in round_moves:
            ...
            move_str = f"{id_drone}-{z_to} Zone {z_to} : {occ_zone}/{max_zone_str} | {link_name}: {occ_link}/{max_link_str}"
    ...
    def update_drones(self, dt: float) -> None:
        ...
        for move in self.schedule[self.current_round]:
            ...
            while first_round > 0 and any(m[0] == id_drone and m[1] == zone_from and m[2] == zone_to for m in self.schedule[first_round - 1]):
                first_round -= 1

            last_round = self.current_round
            while last_round < len(self.schedule) - 1 and any(m[0] == id_drone and m[1] == zone_from and m[2] == zone_to for m in self.schedule[last_round + 1]):
                last_round += 1
    ...
    def compute_schedule(self) -> list[list[tuple[str, str, str]]]:
        ...
        schedule: list[list[tuple[str, str, str, str, bool]]] = [[] for _ in range(max_t)]
        ...
        for d_id, moves in drone_paths.items():
            for step_t, u, v, cost in moves:
                if u != v:
                    for k in range(cost):
                        if k == 0:
                            # 1er tour : franchissement de la liaison inter-zones
                            link_name = f"{u}-{v}"
                            is_link = True
                        else:
                            # Tours suivants : temps de présence au sein de la zone u
                            link_name = f"{u}-{u}"
                            is_link = False
                        schedule[step_t + k].append((d_id, u, v, link_name, is_link))
                else:
                    # Attente stationnaire dans la zone u
                    link_name = f"{u}-{u}"
