...
Attributes:
    ...
    kind: Rôle structurel de la zone, par exemple ``start``, ``normal`` ou ``end``.
    zone_type: Type fonctionnel de la zone (normal, blocked, restricted ou priority).
...
Un nom doit être non vide, ne contenir aucun espace et ne pas contenir de tiret.
...
class HubConfig:
    ...
    La classe garantit l'unicité des noms de zones, l'unicité des coordonnées (x, y)
    ...
    def __init__(self) -> None:
        ...
        self._occupied_coords: dict[tuple[int, int], Zone] = {}  # Pour détecter les chevauchements
        self.start_name: str | None = None
        self.end_name: str | None = None
    
    def validate(self) -> None:
        """Vérifie que le fichier contient au moins un point de départ et d'arrivée."""
    ...
    def add_zone_entry(self, entry: OneLine) -> None:
        ...
        # On laisse l'erreur de parsing remonter (sans try...except local si on veut stopper le script)
        ...
        if coords in self._occupied_coords:
            ...
            print(
                f"Line {entry.count_line}: zone '{zone.name}' overlaps at coordinates "
                f"({zone.x}, {zone.y}) with existing zone '{existing_zone.name}' "
