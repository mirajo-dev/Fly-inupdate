...
@dataclass
class OneLine:
    """Représente une directive non interprétée issue d'une ligne de configuration.
...
@dataclass
class ReadFile:
    """Fournit la lecture et le découpage initial des fichiers de configuration.
...
Chaque ligne non vide et non commentée doit contenir ``:``. La partie gauche est
utilisée comme clé et la partie droite comme valeur, après suppression des espaces
superflus.
...
Raises:
    ValueError: Si une ligne active ne contient pas ``:``, si sa clé est vide ou
        si une autre erreur de validation de format est rencontrée.
    OSError: Peut être rencontré lors de l'ouverture du fichier ; l'implémentation
