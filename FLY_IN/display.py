...
# (t_depart, zone_from, zone_to, cout) : une étape du chemin d'un drone
PathStep = tuple[int, str, str, int]
# (id_drone, zone_from, zone_to, nom_liaison, est_une_liaison) : une entrée
# de planning affichable/animable pour un tour donné
...
class DroneManager:
    ...
    def _format_cap(self, cap: int) -> str:
        """Formate la capacité : remplace la valeur illimitée 999999
        par le nombre total de drones."""
        if cap >= 999999:
    ...
    def format_round(self, round_idx: int,
        ...
        """Méthode gardée en réserve pour le live coding si
        l'évaluation exige le format détaillé."""
...
class Renderer:
    ...
    def draw_nodes(self) -> None:
        for zone in self.zones:
            ...
            try:
                tint_color = Color(zone.color if zone.color else "White")
            except ValueError:
