...
Les métadonnées sont séparées par des espaces.
Chaque élément doit contenir un
signe ``=`` et les clés doivent être uniques.
Une chaîne vide ou l'absence de bloc
produit un dictionnaire vide.

Args:
    raw: Texte brut du bloc de métadonnées.
    line_no: Numéro de ligne utilisé pour produire
...
Raises:
    ValueError: Si le bloc n'est pas correctement délimité,
        contient un token
...
Cette fonction centralise la validation des paramètres
numériques positifs de la
...
Raises:
    ValueError: Si la valeur n'est pas un entier ou si elle
