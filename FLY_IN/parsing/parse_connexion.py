...
"""
    ...
    def get_zones(self) -> tuple[str, str]:
        """Retourne les deux extrémités de la connexion
...
Le format de base est ``<zone1>-<zone2>``
avec un bloc optionnel de métadonnées.
...
Attributes:
    _zones: Dictionnaire des zones connues, utilisé pour
    vérifier les références.
    connections: Liste des connexions valides.
    _seen: Dictionnaire des connexions déjà rencontrées
    avec leur ligne source.
"""
...
Args:
    zones: Dictionnaire des zones déjà déclarées.
    Les connexions doivent référencer
...
Chaque extrémité doit exister dans le dictionnaire des zones.
Une clé ``frozenset``
...
Raises:
    ValueError: Si une zone référencée n'existe pas ou
    si la connexion a déjà été
        déclarée.
"""
        try:
