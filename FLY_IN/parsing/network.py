...
Les identifiants sont générés séquentiellement sous la forme
``D1``, ``D2``, etc.
...
Raises:
    ValueError: Si aucune connexion correspondante n'est trouvée.
"""
...
Le parseur délègue la lecture des lignes à ``ReadFile``,
la validation du nombre
...
Returns:
    Objet ``Network`` contenant les drones, zones, départ,
    arrivée et connexions.

Raises:
    ValueError: Si une directive est inconnue ou si une partie
    de la configuration
